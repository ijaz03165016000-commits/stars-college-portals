...
function wireForm(user, done) {
  ...
  $("#subf").onsubmit = async (e) => {
    ...
    btn.disabled = true;
    try {
      const DUP = "This Transaction ID has already been submitted. If you think this is a mistake, contact the college office.";
  ...
  if (pending) {
    el.innerHTML = `<div class="paywall">
      <div class="paywall__card" style="text-align:center">
        ...
        <p class="muted">Thank you, ${esc(first)}. Your ${money(pending.amount)} payment (TID <b>${esc(pending.tid)}</b>, from ${esc(pending.sender)}) is being verified. This usually takes a few hours during college days.</p>
  ...
  el.innerHTML = `<div class="paywall">
    <div class="paywall__card">
      <div class="paywall__top">
        ...
        <p class="muted" style="margin:0">${expired ? `It expired on ${fmtDate(user.subscribedUntil)}. Renew to keep using the portal.` : `Activate your STARs student portal for just <b>${money(SUB.amount)} a month</b>.`}</p></div>
      </div>
      ...
      <div class="paywall__grid">
        <div>
          ...
          <ul class="perks">
            ...
            <li>${icon("book")}Homework diary</li><li>${icon("calendar")}Class timetable</li><li>${icon("money")}Fee challans and receipts</li><li>${icon("bell")}College notices and leave requests</li>
          </ul>
        </div>
        <div>${payBox()}</div>
      </div>
...
export function subscriptionsAdmin(byName) {
  return async function subscriptions({ el, params, refreshCounts }) {
    ...
    ${card(`${list.length} payment${list.length === 1 ? "" : "s"}`, list.length ? `<div class="tbl-wrap"><table class="tbl"><thead><tr><th>Student</th><th>TID</th><th>Paid from</th><th class="num">Amount</th><th>Submitted</th><th>Status</th><th></th></tr></thead><tbody>
      ${list.map((s) => `<tr><td>${person(s.name, `${s.studentId} · Class ${shortName(s.classId)}`)}</td><td><code>${esc(s.tid)}</code></td><td class="nowrap">${esc(s.sender)}</td><td class="num">${money(s.amount)}</td><td class="nowrap">${fmtDate(s.submittedAt)}</td>
