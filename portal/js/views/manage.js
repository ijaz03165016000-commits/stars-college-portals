...
import { CLASSES, classById, className, shortName, MONTHLY_FEE, FEE_DUE_DAY, LATE_FEE, DAYS, PERIODS } from "../school.js";
import {
...
export function schoolCharts(s) {
  ...
  const groups = {};
  s.students.forEach((st) => { const g = classById(st.classId)?.level >= 11 ? classById(st.classId).group : classById(st.classId)?.level === 8 ? "Class 8" : "Matric " + (classById(st.classId)?.group || ""); groups[g] = (groups[g] || 0) + 1; });
...
async function studentCard(s) {
  ...
  dialog({
    ...
    <dl class="dl" style="margin-bottom:16px"><dt>Father</dt><dd>${esc(s.fatherName)}</dd><dt>Phone</dt><dd>${esc(s.phone)}</dd><dt>Date of birth</dt><dd>${fmtDate(s.dob)}</dd><dt>Address</dt><dd>${esc(s.address)}</dd><dt>Admitted</dt><dd>${fmtDate(s.admissionDate)}</dd><dt>Parent login</dt><dd>${esc(s.parentId || "—")}</dd></dl>
...
function editStudent(s, all, parents, done) {
  ...
  const d = dialog({
    ...
    body: `<div class="form">
      ...
      ${isNew ? `
      ${field("s-id", "Student ID (login)", `<input id="s-id" name="sid" required placeholder="e.g. STR-9B-09" style="text-transform:uppercase"><div class="field__hint">Used to sign in. Can't be changed later.</div>`)}
      ...
      ` : field("s-status", "Status", `<select id="s-status" name="status">${options([["active", "Active"], ["left", "Left the college"]], s.status)}</select>`)}
    </div>`,
    onSubmit: async (f) => {
      const data = { name: f.name.value.trim(), fatherName: f.fatherName.value.trim(), classId: f.classId.value, rollNo: Number(f.rollNo.value), gender: f.gender.value, dob: f.dob.value, phone: f.phone.value.trim(), admissionDate: f.admissionDate.value, address: f.address.value.trim() };
  ...
  $("#s-class", d).onchange = (e) => { if (isNew) $("#s-roll", d).value = nextRoll(e.target.value); };
}
  ...
  return async function fees({ el, params }) {
    ...
    $("#gen").onclick = () => dialog({
      ...
      body: `<div class="form">${field("g-m", "Month", `<select id="g-m" name="month">${options(months.map((m) => [m, fmtMonth(m)]), month)}</select>`)}
        ...
        <p class="muted small">Creates one challan per active student using the monthly fee in <code>school.js</code> (Class 8: ${money(MONTHLY_FEE[8])}, Matric: ${money(MONTHLY_FEE[9])}, Inter: ${money(MONTHLY_FEE[11])}), due on the ${FEE_DUE_DAY}th. Students who already have a challan for that month are skipped.</p>`,
      onSubmit: async (f) => {
        ...
        for (const s of targets) {
          const amount = MONTHLY_FEE[classById(s.classId).level];
          await store.set("fees", `${s.id}_${m}`, { studentId: s.id, classId: s.classId, month: m, amount, due: `${m}-${String(FEE_DUE_DAY).padStart(2, "0")}`, items: [{ label: "Tuition fee", amount }], status: "unpaid", paidOn: "", receiptNo: "", challanNo: `CH-${m.replace("-", "")}-${s.id.replace(/^STR-/, "")}` });
        }
