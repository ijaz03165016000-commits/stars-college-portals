...
async function home({ el }) {
  ...
  el.innerHTML = `
  <div class="hello"><div><h2>College office</h2><p>Manage admissions, staff, logins, fees and timetables. ${fmtDate(today())}</p></div>
    ...
    <svg class="hello__star" viewBox="0 0 24 24"><path d="M12 17.27 18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg></div>
...
async function accounts({ el, params }) {
  ...
  $("#addU").onclick = () => dialog({
    ...
    body: `<div class="form">${field("u-role", "Role", `<select id="u-role" name="role">${options([["admin", "Admin (college office)"], ["principal", "Principal"]])}</select>`)}
