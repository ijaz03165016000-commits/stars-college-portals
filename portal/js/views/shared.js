...
/* ---------- time helpers ---------- */
const toMin = (t) => { let [h, m] = t.trim().split(":").map(Number); if (h < 7) h += 12; return h * 60 + m; };
...
export function studentViews(getSid, { forParent = false, header = () => "" } = {}) {
  ...
  async function home({ el, user }) {
    ...
    <div class="hello">
      ...
      <svg class="hello__star" viewBox="0 0 24 24"><path d="M12 17.27 18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>
    </div>
    ...
    <div class="grid grid--main">
      ...
      <div>
        ${card("Summary", `<div class="ring-row">${ring(all.pct, "overall", all.pct >= 85 ? "var(--green)" : "var(--red)")}
          ...
          ${all.pct < 85 && all.total ? `<p class="form-status is-err" style="display:block;margin-top:14px">Attendance is below 85%. BISE requires 75% to appear in board exams.</p>` : ""}`)}
  ...
  async function results({ el, params }) {
    ...
    <div class="grid grid--main">
      <article class="report">
        <div class="report__head"><img src="../assets/img/logo.svg" alt=""><div><h2>${esc(SITE.name)} — Result card</h2><div class="muted small">${esc(SITE.address)} · Affiliated with ${esc(SITE.board)}</div></div></div>
  ...
  async function fees({ el }) {
    ...
    <p class="muted small" style="margin-top:12px">Pay at the college office or deposit the challan at the bank. Late fee of ${money(LATE_FEE)} applies after the due date. Questions: ${esc(SITE.phone)}.</p>`;
...
export function noticeList(list) {
  ...
  return `<ul class="list">${list.map((n) => `<li>${dayBox(n.date)}<div class="list__main"><b>${esc(n.title)} ${n.pinned ? pill("Pinned", "gold") : ""}</b><div>${esc(n.body)}</div><div class="list__meta">From ${esc(n.by || "College Office")}</div></div></li>`).join("")}</ul>`;
}
export const noticesView = (group, header = () => "") => async ({ el }) => {
  el.innerHTML = header() + card("Notices from the college", noticeList(await noticesFor(group)));
};
...
export const profileView = (details) => async ({ el, user, title }) => {
  ...
  el.innerHTML = `<div class="grid grid--2">
    ${card("Profile", `<div class="profile-top" style="margin-bottom:18px">${avatar(user.name, "avatar--lg")}<div><h2 style="margin:0;font-size:1.4rem">${esc(user.name)}</h2><span class="muted">${esc(user.role[0].toUpperCase() + user.role.slice(1))} · ${esc(user.loginId)}</span></div></div>
      ...
      <p class="muted small" style="margin-top:16px">To correct any detail, contact the college office.</p>`)}
