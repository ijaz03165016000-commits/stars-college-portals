...
/* edit: "self" = the student/staff member can fill it; "office" = only the college office; "record" = comes from the main record */
export const FIELDS = {
  student: [
...
export function cardBack(kind, r) {
  return `<div class="idc idc--back idc--${kind}">
    ...
    <div class="idc__backbody">
      <p class="idc__note">This card is the property of ${esc(SITE.name)} and must be carried at all times on campus. It is not transferable.</p>
      ...
      <div class="idc__verify">
        ...
        <div class="idc__verifytext"><b>Verify online</b>${r.verifyCode
          ? `Scan with any phone camera to confirm this card on the college website.<small>Code ${esc(r.verifyCode)}</small>`
          : r.cardIssuedAt ? "Re-issue this card at the college office to add its verification QR code." : "The verification QR code is added when the card is issued."}</div>
      </div>
      <div class="idc__return"><b>If found, please return to:</b>${esc(SITE.address)}<br>${esc(SITE.phone)} · ${esc(SITE.email)}</div>
    </div>
          ...
          ${card(issued ? "Card details" : "Complete your details", `
            ...
            <form id="cf" class="card-form" novalidate>
              ${FIELDS[kind].map((f) => {
                ...
                return `<div class="cf-row">${tick}<div style="flex:1"><div class="cf-label">${f.label}</div><div class="cf-val">${esc(shown(f) || "—")}${!ok ? ` <span class="muted small">— ${f.edit === "office" || f.edit === "record" ? "the college office will add this" : ""}</span>` : ""}</div></div></div>`;
              }).join("")}
            ...
            ${issued && !office ? `<p class="muted small" style="margin-top:14px">Your card has been issued. To change any detail, contact the college office.</p>` : ""}
