...
export function lineChart(points, { h = 150 } = {}) {
  ...
  return `<svg class="spark" viewBox="0 0 ${w} ${h}" role="img" aria-label="Result trend">
    ...
    <path d="${area}" fill="rgba(242,169,0,.14)"/><path d="${line}" fill="none" stroke="#1D4577" stroke-width="2.5" stroke-linejoin="round"/>
    ${xy.map(([x, yy], i) => `<circle cx="${x}" cy="${yy}" r="4.5" fill="#fff" stroke="#1D4577" stroke-width="2.5"/><text x="${x}" y="${yy - 10}" text-anchor="middle" style="fill:#0B2545;font-weight:700">${points[i][1]}%</text><text x="${x}" y="${h - 6}" text-anchor="middle">${esc(points[i][0])}</text>`).join("")}
  </svg>`;
}
