...
export const LOGIN_DOMAIN = "portal.starscollege.edu.pk";
export const idToEmail = (id) => (String(id).includes("@") ? String(id).trim() : `${String(id).trim().toLowerCase()}@${LOGIN_DOMAIN}`);
...
/* patch keys may be dotted paths, e.g. { "records.STR-9B-01": "P" } */
...
export async function signIn(loginId, password) {
  ...
  if (IS_LIVE) {
    ...
    const profile = await get("users", cred.user.uid);
    if (!profile) { await au.signOut(auth); throw new Error("This account has no portal profile yet. Ask the college admin."); }
    return profile;
  }
  await ensureSeed();
  ...
  if (u.disabled) throw new Error("This account is switched off. Contact the college office.");
