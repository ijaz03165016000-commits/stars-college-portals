...
function notFound() {
  top.innerHTML = `<h1>Article not found</h1><p>This article may have been moved or removed.</p><a class="btn btn--navy" href="articles.html">Browse all articles</a>`;
  body.innerHTML = `<div style="height:60px"></div>`;
}

function setMeta(a) {
  document.title = `${a.title} | STARs College`;
...
getArticle(id).then((a) => {
  ...
  top.innerHTML = `
    ...
    <div class="article-meta"><span>By ${esc(a.author || "STARs College")}</span><span>${formatDate(a.date)}</span><span>${readingTime(a.content)}</span></div>`;
  body.innerHTML = `
